...
public class RulesController extends AbstractController {
	...
	public ModelAndView preAdd() throws Exception {
		...
		if(null != ar){
			mav.addObject("url", "/portalmanagement/rules/update");
		}else{
			mav.addObject("url", "/portalmanagement/rules/add");
		}
		return mav;
	}
	...
	public String update(AuthRulePO bean,String sql,HttpServletRequest request){
		...
		if(blstr.length>0){
			...
			//过滤出重复的数据
			String [] onlyOneStr=new String[blstr.length];
			
			for(int j=0;j<blstr.length;j++){
				boolean flag=true;
				for(int n=0;n<onlyOneStr.length;n++){
					if(blstr[j].equals(onlyOneStr[n])){
						flag=false;
						break;
					}else{
						flag=true;
			...
			//重新插入操作
			Date date=new Date();
			for(int j=0;j<onlyOneStr.length;j++){
				if( null != onlyOneStr[j]){
					BlacklistPO bl1=new BlacklistPO();
					bl1.setCreatetime(date);
					bl1.setUsername(onlyOneStr[j]);
					blackListService.saveBL(bl1);
				}				
			}			
		}
